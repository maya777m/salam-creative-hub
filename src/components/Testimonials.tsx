...
const Testimonials: React.FC = () => {
  ...
  const renderStars = (rating: number) => {
    return Array(5).fill(0).map((_, i) => <Star key={i} size={18} className={`${i < rating ? 'text-amber-500 fill-amber-500' : 'text-gray-300'}`} />);
  };
  return <section id="testimonials" className="py-16 md:py-20 bg-gradient-to-br from-amber-50 via-white to-gray-50 relative overflow-hidden">
      {/* Texture Background */}
      ...
      <div className="container mx-auto px-4 max-w-7xl relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
          {testimonials.map((testimonial, index) => <div key={testimonial.id} className="bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-500 p-8 relative transform hover:-translate-y-2 border border-gray-100" style={{
          animationDelay: `${index * 0.1}s`
              ...
              <div className="flex items-center mb-6">
                ...
                <div>
                  <h4 className="font-semibold text-lg text-gray-800 text-left">{testimonial.name}</h4>
