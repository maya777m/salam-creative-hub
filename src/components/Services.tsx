  ...
  return <section id="services" className="section-padding bg-gray-50">
      <div className="container">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-12">
          {services.map(service => <div key={service.id} className="card group hover:border-l-4 hover:border-amber-500 px-0 my-0 mx-0 py-[10px]">
              <div className="mb-5">{service.icon}</div>
              <h3 className="text-xl font-semibold mb-3">{service.title}</h3>
              <p className="text-gray-600 mb-5">{service.description}</p>
              
              <ul className="mb-6 space-y-2">
                {service.items.map((item, index) => <li key={index} className="flex items-center">
                    <div className="w-2 h-2 rounded-full bg-amber-500 mr-2"></div>
                    <span className="text-gray-600">{item}</span>
                  </li>)}
              </ul>
              
              <Link to="/services" className="inline-flex items-center text-amber-500 font-medium hover:underline group-hover:translate-x-1 transition-transform">
